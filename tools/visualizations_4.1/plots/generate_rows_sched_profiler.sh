...
generate_sched_profiler_graphs_all_parallel()
{
    ...
    cat $INPUT |                                                               \
        ...
        $3 100000 -1 0 standard $1 $2 $4 &

    cat $INPUT |                                                               \
        ${COMPILER}                                                            \
        ./parse_rows_sched_profiler.php                                        \
        output/${5}_load.png                                                   \
        $3 100000 -1 0 load $1 $2 $4 &
}

# Generic runqueue and load graphs
generate_sched_profiler_graphs_all_parallel 0 -1 1 nothing generic

# Considered wakeups for core zero
generate_sched_profiler_graphs_all_parallel 201 0 1 nothing wakeups

# Graph with threads movements and "overloaded wakeups"
generate_sched_profiler_graphs_all_parallel 0 -1 1 arrows arrows
generate_sched_profiler_graphs_all_parallel 0 -1 1 bad_wakeups               \
                                                     overloaded_wakeups
